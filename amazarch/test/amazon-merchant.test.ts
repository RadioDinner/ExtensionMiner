...
describe("collectAmazon", () => {
  const payload = {
    allTransactions: {
      totalCount: 812,
      results: [
        { id: "1", date: "2026-07-10", amount: -24.99, name: "Amazon — old", notes: "existing", merchant: { name: "AMZN Mktp US*1A2B3C" } },
  ...
  it("extracts Amazon rows with integer-cent amounts and reports totals", () => {
    ...
    expect(out.rows).toEqual([
      // row 1 carries a custom display name; row 3 falls back to the merchant name
      { id: "1", date: "2026-07-10", amountCents: -2499, merchantName: "AMZN Mktp US*1A2B3C", name: "Amazon — old", notes: "existing" },
      { id: "3", date: "2026-07-08", amountCents: -11900, merchantName: "Amazon.com", name: "Amazon.com", notes: "" },
    ]);
  });
